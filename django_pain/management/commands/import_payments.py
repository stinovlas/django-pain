...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        parser_module = module_loading.import_string(options['parser'])
        if not issubclass(parser_module, AbstractBankStatementParser):
            raise CommandError('Parser argument has to be subclass of AbstractBankStatementParser.')
        parser = parser_module()  # type: AbstractBankStatementParser
